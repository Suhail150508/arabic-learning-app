...
<style>
    ...
    #allLettersTwoGrid .letter-card {
        margin-bottom: 1.5rem;
    }

    #allLettersTwoGrid .letter-card > a > div {
        border: 1px solid #e9ecef;
        transition: all 0.3s ease;
        min-height: 180px;
    }

    #allLettersTwoGrid .letter-card:hover > a > div {
        border-color: #667eea;
        box-shadow: 0 4px 12px rgba(102, 126, 234, 0.15);
    }

    /* Image containers */
    #allLettersTwoGrid .ratio-1x1 {
        border-radius: 8px;
        background: #f8f9fa;
    }

    #allLettersTwoGrid img {
        transition: transform 0.3s ease;
        object-fit: cover;
    }

    #allLettersTwoGrid .letter-card:hover img {
...
<div class="row g-4">
    ...
    <div class="col-12 col-lg-3 d-flex flex-column align-items-center gap-4 h-80">
        ...
        <button id="btnImage" class="circle-btn" title="{{ __('messages.makhraj') }} (Images)">
            ...
            {{-- <svg width="28" height="28" viewBox="0 0 24 24"><path d="M21 19V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v14l4-4 4 4 4-4 6 6z"/></svg> --}}
        </button>
...
<div class="mt-5 items">
    ...
    <div id="allLettersTwoGrid" class="row"></div>
</div>
@endsection
...
(function(){
    ...
    // Load all letters
    const grid=document.getElementById('allLettersTwoGrid');
    ...
    function renderLetters(list){
        grid.innerHTML = list.map(l=>{
            ...
            <div class="col-6 col-sm-4 col-md-3 col-lg-2 letter-card m-2">
                ...
                <a class="text-decoration-none" href="/letters-two/${l.id}">
    ...
    async function loadAll(){
        try{
            const res=await axios.get('{{ route('api.letters-two') }}', { params: { per_page: 200 } });
